document.addEventListener('DOMContentLoaded', () => {
	...
	(async function checkRemembered(){
		try{
			console.log('signup: checking remembered session...', {api: API_BASE + '/api/current'});
			const res = await fetch(API_BASE + '/api/current', {credentials:'include'});
			const data = await res.json().catch(()=>({}));
			console.log('signup: /api/current response', data, 'document.cookie=', document.cookie, 'localStorage.fizzle_current=', localStorage.getItem('fizzle_current'));
			if (data && data.user){
	...
	form.addEventListener('submit', (e) => {
		...
		.then(async (res) => {
			...
			// Redirect to the main page to show the message app
			window.location.href = '../';
		})
		.catch(err => {
